...
public:
	...
	void setXoffSet(float xoff) { // sets the X axis offset of the map but clamps it at the edge of the map
		XoffSet = xoff;
		XoffSet = Tmpl8::Clamp(XoffSet, 0.0f, width / 3.0f - ScreenWidth / TileLength);
	}
	...
	void setYoffSet(float yoff) {// sets the Y axis offset of the map but clamps it at the edge of the map
		YoffSet = yoff;
		YoffSet = Tmpl8::Clamp(YoffSet, 0.0f, height - ScreenHeight / (float)TileLength);
	}
	...
	void setColliders(float strtX, float strtY, float length, float height, char sign); // editing the map objects function
